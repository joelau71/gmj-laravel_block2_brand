<x-admin.layout.app>
    ...
    $breadcrumbs = [
        ...
        ['name' => $element->title],
        ['name' => "Brand", "link" => route('LaravelBlock2Brand.index', $element->id)],
        ['name' => "Config Edit"]
    ];
    @endphp
    ...
    <form
        class="relative mt-7"
        method="POST"
        action="{{ route("LaravelBlock2Brand.config.update", $element->id) }}"
    >
        @csrf
        @method("patch")
        ...
        <x-admin.atoms.row>
            ...
            <x-admin.atoms.select name="layout" id="layout">
                @foreach (config("gmj.laravel_block2_brand_config.layouts") as $item)
